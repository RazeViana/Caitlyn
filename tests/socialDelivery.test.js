...
test("Discord social replies preserve originals, suppress mentions, and use a stable nonce", async () => {
	...
	assert.equal(payload.content, "Shared by <@444>\noriginal");
...
test("link-only sources send just attribution and keep their source link inside the embed", async () => {
	for (const content of ["https://x.com/alice/status/123", "https://twitter.com/alice/status/123/video/1?share=1"]) {
		...
		const embeds = [{ title: "View post on X", url: "https://x.com/alice/status/123", footer: { text: SOCIAL_PREVIEW_FOOTER } }];
