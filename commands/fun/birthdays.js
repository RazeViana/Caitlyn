	...
	async execute(interaction) {
		...
		try {
			...
			const birthdayListEmbed = new EmbedBuilder()
				...
				.setFooter({ text: 'ggwp old bastards', iconURL: 'https://i.imgur.com/n8FlP3v.png' });
			allBirthdays.forEach(birthday => {
				birthdayListEmbed.addFields({ name: birthday.name, value: format(birthday.dob, 'dd MMMM yyyy') });
			});
