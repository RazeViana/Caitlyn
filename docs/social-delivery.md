...
Status: implemented on `codex/social-media-replacement`; the owner authorized a local Discord test on 2026-09-12. The current Mac test process has integration enabled, migration `014` applied locally, and guild commands registered. Channels still require administrator opt-in. Nothing is merged, pushed, or deployed to the homeserver. The X adapter/renderer checkpoint is signed as `0f3dee4`; the delivery integration is subsequent uncommitted work. This is the first platform slice of the full replacement, not completed Instagram/TikTok/Reddit support.

## Behavior and configuration

The old hosted-fixer path has been removed. New previews are standalone messages containing `Shared by @sender`, the caption without handled X URLs, our attributed embeds, and verified files. Link-only sources produce only the sender attribution above the card. The original post remains available through the clickable `View post on X` title and author line; quotes have their own separately attributed card/link. Author names are plain text without visible Markdown escapes, and media-only posts omit the empty-text placeholder. Videos remain playable uploaded attachments alongside the card: Discord's classic bot-created rich embeds do not accept a custom `video` field ([message creation limits](https://docs.discord.com/developers/resources/message#create-message)).

Caption cleanup uses the same recognition and five-link limit as admission. It removes all visible aliases of handled X links, keeps named Markdown link labels, and preserves unrelated/unsupported links, hidden/code/suppressed content and links beyond the admission limit. Those preserved links and genuine URLs in the extracted post text are not indiscriminately removed. Full-text attachments retain attribution and source URLs. New main-card footers say only `Caitlyn preview`, without a job ID; IDs remain internal to the database/logging/send metadata.

Only the original sender is allowed to receive a mention; provider text and other source mentions cannot ping roles or everyone. Once every X preview from a source is confirmed complete, Caitlyn removes the unchanged original if it has Manage Messages. Failed/partial previews and messages containing uploads, stickers, polls, reply context, a thread, or text too long to copy retain the original. Old jobs are not retroactively cleaned up. The owner confirmed the authenticated video path works on September 13; the new presentation awaits a fresh owner-sent link.
...
`/social disable` cancels pending jobs in the current channel; `/social disable-server` does so throughout the current guild. Existing completed previews are left in place unless their source changes. `/social status` reports saved enabled channels and whether the operator enabled integration, **not** a worker-health probe. The command is guild-only and administrator-only, with ephemeral confirmations. Threads, DMs, announcement channels, unresolved share links, and non-X adapters are not supported yet. Command publication is separate from startup; it has been performed only for the authorized local test's guild.
...
Uploads use a conservative 8 MiB/file and 20 MiB aggregate budget, not a guild boost-tier calculation. The renderer additionally reserves 128 KiB for payload/long-text overhead. Large or unavailable files are explicitly omitted; originals remain accessible. No transcoding, persistent media cache, paid API, or hosted-fixer fallback is added. Account authentication is confined to the FxEmbed backend as described above.
...
2. Post a fresh X image or text link. Caitlyn should mention the sender, preserve the caption without the handled raw X URLs, send the complete preview, then remove the original. Check that `View post on X` opens the source, author handles have no visible escape characters, and a link-only source has no extra URL line. Try several links and verify all are delivered before deletion; try a video and check playback/audio on desktop and mobile. Failed/partial media must retain the source.
